...
export const register = async (req, res) => {
  ...
  const existingUser = await User.findOne({ email });
    ...
    res.status(201).json({ message: "User registered successfully", user: { name: responseUser.name, email: responseUser.email } });
  } catch (error) {
...
export const login = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Login successful", user: { name: responseUser.name, email: responseUser.email } });
  } catch (error) {
